...
public function search(int $id, Request $request, PostRepository $postRepository, CategorieForumRepository $categorieForumRepository): Response
{
    ...
    // Construire la requête de base avec la catégorie
    $queryBuilder = $postRepository->createQueryBuilder('p')
        ->where('p.categorieForum = :categorie')  // Changé de 'forum' à 'categorieForum'
        ->setParameter('categorie', $categorieForum);

    // Si une recherche est spécifiée, ajouter la condition de recherche
    if (!empty($query)) {
        $queryBuilder->andWhere('p.title LIKE :query')
            ->setParameter('query', '%' . $query . '%');
    }
